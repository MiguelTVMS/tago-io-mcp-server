...
const userListSchema = querySchema.extend({
  filter: z
    .object({
      id: z
        .string()
        .describe("Filter by user ID. E.g: '123456789012345678901234'")
      ...
      tags: z
        ...
        .describe("Filter by tags. E.g: [{ key: 'user_type', value: 'admin' }]")
        .optional(),
    ...
    .describe(
      'Filter object to apply to the query. Available filters: id, name, email, active, tags'
    )
    .optional(),
});
...
const userBaseSchema = z
  .object({
    operation: z
      .enum(['lookup'])
      .default('lookup')
      .describe('The type of operation to perform on the user.'),
    runUserID: z
      .string()
      .describe(
        'The ID of the user to perform the operation on. Optional for lookup and create, but required for update and delete operations.'
      )
      .length(24, 'ID must be 24 characters long')
      .optional(),
    // Separate fields for different operations to maintain type safety
    lookupUser: userListSchema
      .describe('The user to be listed. Required for lookup operations.')
      .optional(),
  })
...
async function userOperationsTool(resources: Resources, params: UserSchema) {
  ...
  const factory = createOperationFactory<UserSchema>().register('lookup', (params) =>
