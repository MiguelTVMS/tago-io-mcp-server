...
const entityListSchema = querySchema.extend({
  filter: z
    .object({
      id: z
        .string()
        .describe("Filter entity by ID. E.g: '123456789012345678901234'")
      ...
      tags: z
        ...
        .describe("Filter by tags. E.g: [{ key: 'entity_type', value: 'sensor' }]")
        .optional(),
...
const entityBaseSchema = z
  .object({
    ...
    entityID: z
      .string()
      .describe(
        'The ID of the entity to perform the operation on. Optional for lookup and create, but required for update and delete operations.'
      )
      .optional(),
    // Separate fields for different operations to maintain type safety
    lookupEntity: entityListSchema
      .describe('The entity to be listed. Required for lookup operations.')
      .optional(),
  })
...
async function entityOperationsTool(resources: Resources, params: EntitySchema) {
  ...
  const factory = createOperationFactory<EntitySchema>().register('lookup', (params) =>
